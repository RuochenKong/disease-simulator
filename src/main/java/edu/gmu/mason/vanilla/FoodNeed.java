...
public class FoodNeed implements Need, java.io.Serializable {
	...
	public void update() {
		...
		// calculate the minute difference since last time eaten which will update the
		// fullness
		int minuteDiff = Minutes.minutesBetween(lastTimeAte,
				agent.getSimulationTime()).getMinutes();

		//// (if statements below shows the effect of eating characteristics on Food
		//// Need Status and fullness)
		switch (this.status) {
			case JustAte:
